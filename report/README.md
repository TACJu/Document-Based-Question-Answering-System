...
本模型使用上述的CNN结构对问题和文档进行相同的处理，得到两个向量x~d~，x~q~。然后计算这两个向量的相似度x~sim~ = x~d~^T^Mx~q~，M是一个由网络学习得到的参数矩阵。将x~d~，x~sim~，x~q~与额外的特征合并为一个向量作为最后全连接层的输入。

<img src="RSTP2.jpg" width="90%" />

我们认为这个简单粗暴的模型有效的原因是：它没有直接计算query和document之间的余弦距离，而是多用了一个参数矩阵M。这种计算相似度的方式可以看作是将x~d~映射成为x~d~^T^=Mx~d~，再让x~d~，x~q~之间计算相似度。
